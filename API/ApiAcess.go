...
import (
	...
	"net/http"
	"time"
    P "dependencia.es/Qrcode/qrcode"
...
func homePage(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		fmt.Println("Algo ha ido muy mal")
		return
	}
	fmt.Println("Esto es lo que viene en el vars id ", key)
	w.WriteHeader(http.StatusOK)
	w.Header().Set("Content-Type", "application/octet-stream")
	w.Write(P.TqrCode(key[0]))

	//fmt.Fprintf(w, P.TqrCode(key[0]))
